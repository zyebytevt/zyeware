...
module zyeware.graphics.camera;
...
public:
    ...
    pragma(inline, true)
    final mat4 getProjectionMatrix() pure const nothrow
        => mat4.orthographic(0, mViewportSize.x, mViewportSize.y, 0, -1, 1);

    pragma(inline, true)
    final mat4 getViewMatrix() pure const nothrow
        => mat4.translation(vec3(-position, 0)) * mat4.zRotation(
            rotation) * mat4.scaling(zoom, zoom, 1);
}
    ...
    final mat4 getProjectionMatrix() pure const nothrow {
        if (projection == Projection.orthographic) {
            return mat4.orthographic(-mViewportSize.x / 2, mViewportSize.x / 2,
                -mViewportSize.y / 2, mViewportSize.y / 2, renderRange.min,
                renderRange.max);
        } else {
