...
return SDLValue.text(value.to!string); else static if (is(T == long))
return SDLValue.long_(value); else static if (isIntegral!T)
return SDLValue.int_(value.to!int); else static if (is(T == double))
return SDLValue.double_(value); else static if (is(T == float))
return SDLValue.float_(value); else static if (is(T == bool))
return SDLValue.bool_(value); else static if (is(T == SysTime))
return SDLValue.dateTime(value); else static if (is(T == Date))
return SDLValue.date(value); else static if (is(T == Duration))
return SDLValue.duration(value); else static if (is(T == ubyte[]))
return SDLValue.binary(value); else static if (isVector!T)
return marshalArray!(T.vt[])(value.vector); else static if (isArray!T)
return marshalArray!T(value); else 
...
return cast(T)value; else static if (is(T == color)) {
if (value.isText)
return color(value.textValue); else 
return color(unmarshalVector!vec4(value));
} else static if (isVector!T)
return unmarshalVector!T(value); else static if (isArray!T)
return unmarshalArray!T(value); else 
...
@("SDLite convenience functions") unittest {
