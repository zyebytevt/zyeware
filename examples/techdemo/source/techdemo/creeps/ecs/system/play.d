...
protected:
    ...
    Entity createPlayer(vec2 position) {
        ...
        color.white);
        player.register!SpriteAnimationComponent(AssetManager.load!SpriteFrames(
                "res:creeps/sprites/player.anim"), "walk", No.autostart);
    ...
    Entity createMob(vec2 position) {
        immutable vec2 targetPoint = vec2(CreepsMenuState.screenSize) / 2 + vec2(
            uniform(-100, 100), uniform(-200, 200));
        ...
        color.white);
        mob.register!SpriteAnimationComponent(AssetManager.load!SpriteFrames(
                "res:creeps/sprites/creeps.anim"),
...
public:
    ...
    in (playState) {
        ...
        mSpawnTimer = new Timer(dur!"msecs"(500), delegate void(Timer timer) {
            ...
            position.x = clamp(CreepsMenuState.screenSize.x / 2 + position.x, 0, CreepsMenuState
                .screenSize.x);
            position.y = clamp(CreepsMenuState.screenSize.y / 2 + position.y, 0, CreepsMenuState
                .screenSize.y);
